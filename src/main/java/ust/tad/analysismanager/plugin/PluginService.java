...
public class PluginService {
   ...
   */
  public Plugin getPluginByTechnologyAndAnalysisType(String technology, AnalysisType analysisType)
    ...
    try {
      ...
      throw new PluginException(
          String.format(
              "No appropriate plugin currently registered for technology: '%s' and analysisType: '%s'.",
              technology, analysisType.toString()));
    }
  }

  /**
   * Gets a Plugin for the given deployment technology. First looks for a static plugin. If there is
   * no static plugin, it looks for a dynamic plugin. When no plugin can be found a PluginException
   * is thrown.
   *
   * @param technology
  ...
  public Plugin getPluginByTechnology(String technology) throws PluginException {
    ...
    Optional<Plugin> plugin =
        plugins.stream().filter(p -> p.getAnalysisType() == AnalysisType.STATIC).findAny();
    if (plugin.isPresent()) {
      return plugin.get();
    } else {
      plugin = plugins.stream().filter(p -> p.getAnalysisType() == AnalysisType.DYNAMIC).findAny();
      return plugin.orElseThrow(
          () ->
              new PluginException(
                  String.format(
                      "No appropriate static or dynamic plugin currently registered for technology: '%s'.",
                      technology)));
    }
  }
}
